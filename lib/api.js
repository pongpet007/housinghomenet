...
export function getAssetList({ asset_type_id, sale_type_id, keyword, is_new, limit, page }) {
    let params = {
    ...
    const { data, error } = useSWR(API_URL + 'asset/getlist?' + queryString, fetcher);
    return data;
}

export function getAssetOne(asset_id) {

    const { data, error } = useSWR(API_URL + `asset/getOne?asset_id=${asset_id}`, fetcher);
    return data;
...
export function getServiceTypeList({limit,page}) {
    ...
    const queryString = Object.keys(params).map(key => key + '=' + params[key]).join('&');
    const { data, error } = useSWR(API_URL + 'servicetype/getlist?' + queryString, fetcher);
    return data;
}

export function getServiceTypeOne(service_type_id) {
    const { data, error } = useSWR(API_URL + `servicetype/getOne?asset_id=${service_type_id}`, fetcher);
    return data;
}
...
export function getServiceList({ service_type_id, limit, page }) {
    ...
    const queryString = Object.keys(params).map(key => key + '=' + params[key]).join('&');
    const { data, error } = useSWR(API_URL + 'service/getlist?' + queryString, fetcher);
    return data;
}

export function getServiceOne(service_type_id) {
    const { data, error } = useSWR(API_URL + `service/getOne?asset_id=${service_type_id}`, fetcher);
    return data;
}
